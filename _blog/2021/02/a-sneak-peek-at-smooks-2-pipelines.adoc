...
_This post was reproduced on the https://oncodesign.io/2021/02/16/a-sneak-peek-at-smooks-2-pipelines/[On Code & Design blog]._

image::/assets/blog/pipeline.jpg[align=center]

footnote:[https://www.flickr.com/photos/54758495@N00/11431784985[“Pembroke Reverse Osmosis Plant pipeline”] by https://www.flickr.com/photos/54758495@N00[Rrrodrigo] is licensed under https://creativecommons.org/licenses/by-nc/2.0/?ref=ccsearch&atype=rich[CC BY-NC 2.0]]

We’re inching ever closer to releasing https://www.smooks.org/[Smooks 2] milestone 3. Featuring in this release will be the powerful ability to declare pipelines. A pipeline is a flexible, yet simple, Smooks construct that isolates the processing of a targeted event from its main processing as well as from the processing of other pipelines. In practice, this means being able to compose any series of transformations on an event outside the main execution context before directing the pipeline output to the execution result stream or to other destinations.
...
____
An organisation is required at the end of day to churn through GBs of customer orders captured in a CSV file. Each individual order is to be communicated to the inventory system in JSON and uploaded as XML to a CRM service. Furthermore, an https://unece.org/trade/uncefact/introducing-unedifact[EDIFACT] document aggregating the orders needs to be exchanged with the supplier.
____

A scalable solution powered by Smooks pipelines can be conceptualised to:

image::/assets/blog/smooks-2-pipelines.png[]
...
The first step to implementing the solution is to turn the CSV file stream into an event stream using the https://daffodil.apache.org/[DFDL] parser added in Smooks 2.0.0-M1 (alternatively, a simpler but less flexible reader from the CSV cartridge can be used for this purpose):
...
The `AfterVisitor` implementation leverages the popular https://github.com/FasterXML/jackson[Jackson library] to serialise the record element into JSON which is then transparently written out to `inventoryOutputStream` with `Stream.out(executionContext).write(...)`.
...
A functional version of the https://github.com/smooks/smooks-examples/tree/master/pipelines[solution is available on GitHub]. Take it for a spin. Your feedback on https://groups.google.com/g/smooks-user[Smook’s user forum] is most welcome.